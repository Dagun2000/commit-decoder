...
_NO_SUBSTANTIVE_CHANGES_BODY_KO = """\
### 1. What (기능적 요약)
...
_NO_SUBSTANTIVE_CHANGES_BODY_EN = """\
### 1. What (Functional Summary)
This commit contains only non-functional changes such as comments, \
whitespace, or typo fixes, so there is no actual change in code behavior.

### 2. How (Implementation Mechanism)
Stage 1 analysis found no structurally changed functions, classes, or \
variables, so there is no implementation mechanism to describe.

### 3. Why (Technical Intent)
This is presumed to be a non-functional change aimed at improving \
readability or tidying up documentation.\
"""

_NO_SUBSTANTIVE_CHANGES_BODIES = {
    "ko": _NO_SUBSTANTIVE_CHANGES_BODY_KO,
    "en": _NO_SUBSTANTIVE_CHANGES_BODY_EN,
}
...
_SYSTEM_PROMPT_EN = """\
You are a senior engineer writing a commit analysis report in English.
...
Write all section content in English, 2-5 concise sentences each. Do not \
add extra sections, a title, or any preamble/postamble text.
"""
...
def generate_report(
    ...
    if not skeleton.strip():
        return _NO_SUBSTANTIVE_CHANGES_BODIES[language]
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": _SYSTEM_PROMPTS[language]},
